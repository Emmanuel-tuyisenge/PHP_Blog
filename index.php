...
if (count($articles)) {
    $categoriesMap = array_map(fn ($a) => $a['category'], $articles);
    $categories = array_reduce($categoriesMap, function ($acc, $cat) {
        if (isset($acc[$cat])) {
            $acc[$cat]++;
        } else {
            $acc[$cat] = 1;
                        ...
                        <?php foreach ($categories as $cat => $num) : ?>
                            ...
                            <div class="articles-container">
                                <?php foreach ($articlePerCategories[$cat] as $a) : ?>
                                    <a href="/show_article.php?id=<?= $a['id'] ?>" class="article block">
                                        <div class="overflow">
